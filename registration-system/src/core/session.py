...
class SessionData:
    ...
    def __init__(self, session_dict: Dict[str, Any]):
        ...
        self.operation_type = session_dict.get("operation_type")
        ...
        self.created_at = session_dict.get("session_created_at")
...
class SessionManager:
    ...
    async def process(self):
        ...
        try:
            ...
            self.session_queue_manager(session_cache)
    ...
    def pre_process(self) -> SessionData:
        """Load session from storage and create session data object"""
        # Create session
        self.create_session()

        if not self.sessions:
            ErrorHandler.raise_session_not_found("No session found")
    ...
    def post_process(self) -> Dict[str, Any]:
        ...
        if response["status"] == "FAILED":
            ...
            if is_removed:
                    response["status"] = "FAILED-OLD-CUSTOMER"
            status = response["status"]
            self.Err = response["message"] if "message" in response else "Business err"

        if not self.Err:
            response = {
                "session_id": self.session_id,
                "status": status,
                "message": "{0} for {1} with id: {2}".format(self.sessions["operation_type"], self.entity_type, self.sessions["phone"])
            }
        else:
            logger.error(f"Error in process: {self.Err}")
            response = {
                "session_id": self.session_id,
                "status": status,
                "message": self.Err
    ...
    def update_entity_meta_data(self):
        ...
        if self.entity_type == "customer":
            ...
            version_no = customers[self.payload.phone]["latest_latest_version"] + 1
            seq_id     = meta_data["metadata"]["total_customers"]
            if not customers[self.payload.phone]["customer_id"]:
                self.entity_key = f'{seq_id + 1}#{self.payload.phone}'
            ...
            customers[self.payload.phone]["versions"].append({version_no : f'{self.payload.business}#{self.payload.operation_id}#{self.session_id}'})
    ...
    def session_queue_manager(self, session_cache: SessionData) -> None:
        """
