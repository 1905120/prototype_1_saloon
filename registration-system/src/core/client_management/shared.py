"""
Shared client cache singleton management
"""
import json
import os
...
_cache_registry: Dict[str, 'ClientCache'] = {}
_registry_lock = Lock()


class ClientCache:
    """Thread-safe client cache for a specific business"""
    ...
    def __init__(self, path: str):
        ...
        Args:
            path: Path to client metadata file
        """
        self.path = path
    ...
    def _load_from_file(self) -> None:
        ...
        try:
            if os.path.exists(self.path):
                ...
                logger.info(f"Loaded client cache from {self.path}")
            else:
                self._data = {}
                logger.info(f"Client cache file not found at {self.path}, starting with empty cache")
            self._loaded = True
        except Exception as e:
            logger.error(f"Failed to load client cache from {self.path}: {str(e)}")
            self._data = {}
            self._loaded = True
...
def get_cache(path: str) -> ClientCache:
    ...
    Returns:
        ClientCache singleton for this path
    """
    with _registry_lock:
        if path not in _cache_registry:
            _cache_registry[path] = ClientCache(path)
            logger.info(f"Created new client cache singleton for path: {path}")
...
def destroy_cache(path: str) -> None:
    ...
    with _registry_lock:
        if path in _cache_registry:
            ...
            logger.info(f"Destroyed client cache singleton for path: {path}")
